...
| GET    | `/apr/:address`                      | get time series of annualized rewards rates calculated per month   | `start (required)` - start date in format `2006-01-02` `end (required)` - end date in format `2006-01-02` `address (required)` - address of account `including_dailies (required)` - returns daily rates |
